...
function createBoard() {
    ...
    container.style.gridTemplateColumns = "80px 80px 80px 80px 80px 80px 80px 80px"
...
document.querySelector("body").insertBefore(undo,document.querySelector("div"))
}
...
function processGame(e) {
    e.preventDefault();
    console.log("id is" + (event.target.getAttribute("id") || event.target.parentElement.getAttribute("id")))
    if (arr.length == 0) {
        arr.push((event.target.getAttribute("id") || event.target.parentElement.getAttribute("id")))
        console.log("pushed into arr" + (event.target.getAttribute("id") || event.target.parentElement.getAttribute("id")))
        console.log("arr is" + arr.length, arr[0])
        storeState();
    }
    else {
        var ele = arr.pop()
        console.log("pop element from arr" + document.getElementById(ele).getAttribute("id"))
        console.log("ele text content is" + document.getElementById(ele).textContent)
